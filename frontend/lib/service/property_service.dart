...
class PropertyService {
  ...
  // Generate Bakong QR
  Future<http.Response> generateBakongQr({required int paymentId}) async {
    final String token = await _getToken();

    final response = await http.post(
      Uri.parse("$baseUrl/payments/$paymentId/generate-qr"),
      headers: {"Accept": "application/json", "Authorization": "Bearer $token"},
    );

    print("GENERATE BAKONG QR STATUS: ${response.statusCode}");

    print("GENERATE BAKONG QR RESPONSE: ${response.body}");
  ...
  Future<http.Response> checkBakongPayment({
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/payments/$paymentId/check"),
      headers: {
        "Accept": "application/json",
    ...
    print("CHECK BAKONG PAYMENT STATUS: ${response.statusCode}");

    print("CHECK BAKONG PAYMENT RESPONSE: ${response.body}");
  ...
  // Send Owner Request
  Future<http.Response> sendOwnerRequest({
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/owner/requests"),
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
    ...
    print("SEND OWNER REQUEST STATUS: ${response.statusCode}");
    print("SEND OWNER REQUEST RESPONSE: ${response.body}");

    return response;
  }

  // Get Owner Requests
  Future<http.Response> getOwnerRequests() async {
    final String token = await _getToken();
    ...
    print("OWNER REQUESTS STATUS: ${response.statusCode}");
    print("OWNER REQUESTS RESPONSE: ${response.body}");

    return response;
  }

  // Mark replied owner requests as seen by the owner
  Future<http.Response> markOwnerRequestNotificationsSeen() async {
    final String token = await _getToken();

    final response = await http.post(
      Uri.parse("$baseUrl/owner/requests/mark-seen"),
      headers: {"Accept": "application/json", "Authorization": "Bearer $token"},
    );

    print(
      "MARK OWNER REQUEST NOTIFICATIONS SEEN STATUS: ${response.statusCode}",
    );
    print("MARK OWNER REQUEST NOTIFICATIONS SEEN RESPONSE: ${response.body}");

    return response;
  }

  // Admin - Get Owner Requests
  Future<http.Response> getAdminOwnerRequests() async {
    final String token = await _getToken();

    final response = await http.get(
      Uri.parse("$baseUrl/admin/owner-requests"),
      headers: {"Accept": "application/json", "Authorization": "Bearer $token"},
    );

    print("ADMIN OWNER REQUESTS STATUS: ${response.statusCode}");
    print("ADMIN OWNER REQUESTS RESPONSE: ${response.body}");

    return response;
  }

  // Admin - Reply To Owner Request
  Future<http.Response> replyOwnerRequest({
    ...
    final response = await http.post(
      Uri.parse("$baseUrl/admin/owner-requests/$requestId/reply"),
      headers: {
        "Accept": "application/json",
        "Content-Type": "application/json",
        "Authorization": "Bearer $token",
    ...
    print("REPLY OWNER REQUEST STATUS: ${response.statusCode}");
    print("REPLY OWNER REQUEST RESPONSE: ${response.body}");
